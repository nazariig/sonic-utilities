...
def fw_update(ctx, yes, force, image):
    ...
    #component = ctx.obj[COMPONENT_CTX_KEY]
    #firmware_update_notification = component.get_firmware_update_notification()
    ...
    try:
        ...
        if not cup.is_firmware_update_available(chassis_name, module_name, component_name):
            ...
            #log_helper.print_info("Firmware update is not available")
            #ctx.exit(EXIT_SUCCESS)
        ...
        if not (cup.is_firmware_update_required(chassis_name, module_name, component_name) or force):
            ...
            #log_helper.print_info("Firmware is up-to-date")
            #ctx.exit(EXIT_SUCCESS)

        firmware_update_notification = cup.get_notification(chassis_name, module_name, component_name)
        if firmware_update_notification:
            log_helper.print_warning(firmware_update_notification)
...
#    cup = ComponentUpdateProvider(chassis_name, module_name, component_name, force)
#    cup.update_firmware()
...
#@click.pass_context
def update1(ctx, yes, force, image):
    """Update platform firmware"""
    aborted = False
...
# 'updates' subcommand
@show.command()
@click.option('-i', '--image', 'image', type=click.Choice(["current", "next"]), default="current", show_default=True, help="Show updates using current/next image")
@click.pass_context
def updates(ctx, image):
    ...
    try:
        cup = ComponentUpdateProvider()
        #cup_get_status()
        click.echo(cup.get_status(False))
    except Exception as e:
        cli_abort(ctx, str(e))
    #try:
